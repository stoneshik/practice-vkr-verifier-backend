...
На данный момент фронтенд и бэкенд настроен на то чтобы бэкенд работал по адресу: http://localhost:3351
...
- Для тестирования используется POSTMAN, импортированная коллекция в формате json - [ссылка](REST_API_VKR_Verifier.postman_collection.json)
- Для нагрузочного тестирования создан специальный профиль `jmeter` и также сценарий `jmeter` - [ссылка](jmeter/files.jmx)
- Также есть подробное описание технического задания в формате pdf - [ссылка](ТЗ_Стрельбицкий_И_П.pdf)
...
Запуск:
```bash
java -jar backend/verifier-0.0.1-SNAPSHOT.jar
```

---

На профиле `prod` ожидается следующая структура рабочей директории проекта (из нее должен вызываться вызов запуска jar-файла):
- backend/verifier-0.0.1-SNAPSHOT.jar - собранный в jar файл бэкенд
- uploads - директория, в которую сохраняются файлы
- verifier - директория с исходниками Python скрипта
- verifier/.venv/bin/python - .venv интерпретатор

На профиле `dev` ожидается следующая структура рабочей директории проекта (в ней содержится рабочие директории исходников):
- verifier-backend (название не важно) - директория в которую склонирован репозиторий бэкенда (именно она является рабочей при запуске бэкенда)
- uploads - директория, в которую сохраняются файлы
- verifier - директория с исходниками Python скрипта
- verifier/.venv/bin/python - .venv интерпретатор

На профиле `jmeter` ожидается такая же структура как у профиля `dev`, важной особенностью является, то что используется режим hibernate `create`. То есть база данных при каждом запуске очищается. Также в нем увеличено количество за раз обрабатываемых файлов до 10 (параметр `batch-size`) и уменьшена задержка между запуском обработчика файлов (параметр `scheduler.fixed-delay-ms`)
